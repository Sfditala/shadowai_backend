...
data = [
    ...
    ("my mastercard number 5105105105105100 cvv 456", "critical"),
    ("بطاقتي الائتمانية رقم 4539578763621486", "critical"),
    ("card details: 4916338506082832 / 05/28 / 123", "critical"),
    ("pay using card 4532015112830366", "critical"),
    ("credit card number is 4716184360702308", "critical"),
    ("بطاقة ماستركارد 5425233430109903", "critical"),
    ("my visa is 4929420209979305 expires 03/27", "critical"),
    ("كارت الدفع: 4111 1111 1111 1111", "critical"),
    ("card no: 5500-0055-5555-5559", "critical"),
    # كلمات مرور إضافية
    ("password : Admin2024!", "critical"),
    ...
    ("كلمة سر الراوتر: Router@Admin1", "critical"),
    ("system password is Sys@Pass123", "critical"),
    ("old password was Old@Pass123 new is New@Pass456", "critical"),
    ("ssh password: SSH@Secure99", "critical"),
    # مفاتيح وتوكنات إضافية
    ("OPENAI_API_KEY=sk-proj-abc123def456ghi789", "critical"),
    ("firebase_api_key=AIzaSyAbCdEfGhIjKlMnOp", "critical"),
    ("twilio_auth_token=abcdef1234567890abcdef", "critical"),
    ("sendgrid_api_key=SG.abc123def456ghi789jkl", "critical"),
    ("cloudinary_api_secret=abcdefghijklmnopqrstu", "critical"),
    ("mapbox_token=pk.eyJ1IjoiYWJjZGVmIn0.xyz123", "critical"),
    ("pusher_app_secret=abcdef123456", "critical"),
    ("paypal_client_secret=AbCdEfGhIjKlMnOpQrSt", "critical"),
    ("GOOGLE_CLIENT_SECRET=GOCSPX-abcdefghijklmno", "critical"),
    ("facebook_app_secret=abcdef1234567890", "critical"),
    ("discord_bot_token=MTA0abc.def.ghijklmnop", "critical"),
    ("telegram_bot_token=1234567890:ABCdefGHI", "critical"),
    ("مفتاح Stripe: sk_live_abcdefghijklmnop", "critical"),
    ("توكن Firebase: ya29.abcdefghijklmnop", "critical"),
    ("heroku_api_key=abcdef12-3456-789a-bcde-f0123456789a", "critical"),
    ("digitalocean_token=dop_v1_abcdef1234567890", "critical"),
    ("linode_api_key=abcdef1234567890abcdef12", "critical"),
    ("vultr_api_key=ABCDEF1234567890ABCDEF12", "critical"),
    ("cloudflare_api_token=abcdefghijklmnopqrstu", "critical"),
    ("npm_token=npm_abcdefghijklmnopqrstuvwxyz", "critical"),
    ("docker_password=Docker@Secret123", "critical"),
    ("private_key = MIIEvgIBADANBgkqhkiG9w0B", "critical"),
    ("SECRET=super_secret_key_123456789", "critical"),
    ("APP_SECRET=myappsecret123456", "critical"),
    ("ENCRYPTION_KEY=32charslongsecretkey123456", "critical"),
    ...
    ("رقم بطاقة الصراف: 4532 0151 1283 0366", "critical"),
    ("credit: 4916-3385-0608-2832 exp: 12/26", "critical"),
    ("بطاقة الائتمان الخاصة بي 4539 5787 6362 1486", "critical"),
    ("رمز الـ OTP السري: 847291", "critical"),
    ("كود التحقق: 293847", "critical"),
    ("one time password: 582930", "critical"),
    ("verification code: 748291", "critical"),
    ("pin code: 9284 للبطاقة", "critical"),
    ("الرقم السري للبطاقة: 4829", "critical"),
    ("cvv البطاقة: 392", "critical"),
    ("card cvv is 847", "critical"),
    ("security code: 293", "critical"),
    ("رمز الأمان للبطاقة: 748", "critical"),
    ("DATABASE_PASSWORD=MyDbPass@2024", "critical"),
    ("REDIS_PASSWORD=Redis@Secret123", "critical"),
    ("MONGO_PASSWORD=MongoPass@2024", "critical"),
    ("MYSQL_ROOT_PASSWORD=Root@MySQL2024", "critical"),
    ...
    ("wallet password: Wallet@Crypto1", "critical"),
    ("seed phrase: apple banana cherry dog elephant fox", "critical"),
    ("مفتاح المحفظة: word1 word2 word3 word4 word5 word6", "critical"),
    ("metamask seed: abandon ability able about above absent", "critical"),
    ("private key للمحفظة: 5KJvsngHeMpm884wtkJNzQGaCErckhHJ", "critical"),
    ("sk_test_4eC39HqLyjWDarjtT1zdp7dcNewKey123456", "critical"),
    ("stripe key: sk_live_newkey123456789abcdef", "critical"),
    ("app_secret=MyAppNewSecret2024!", "critical"),
    ("jwt_secret=NewJwtSecret@2024Secure", "critical"),
    ("encryption key: NewEncKey@2024Abc", "critical"),
    ("signing_key=SigningKey@2024Secure", "critical"),
    ("webhook_secret=NewWebhook@Secret1", "critical"),
    # ================================================================
    # 🔴 HIGH — إضافة 100 جملة
    # ================================================================
    # هوية وجوازات إضافية
    ...
    ("رقم بطاقة الهوية: 1234567890", "high"),
    ("identity number: 0987654321", "high"),
    ("رقم الهوية: 5678901234", "high"),
    ("ID number 2345678901", "high"),
    ("هويتي رقم 3456789012", "high"),
    ("personal ID: 4567890123", "high"),
    ("رقم وثيقة السفر: 567890123", "high"),
    ("travel document number: 678901234", "high"),
    ("رقم الجواز: PA9876543", "high"),
    ("passport no: BC1234567", "high"),
    # إيميلات إضافية
    ("ايميلي هو: mohammed@hotmail.com", "high"),
    ("my work email: sarah.johnson@bigcorp.com", "high"),
    ("تواصل معي: fatima.ali@outlook.com", "high"),
    ("personal email: user123@yahoo.com", "high"),
    ("بريدي: ahmad.khalil@gmail.com", "high"),
    ("email me at: contact@mywebsite.com", "high"),
    ("الإيميل الرسمي: official@ministry.gov.ps", "high"),
    ("student email: s123456@student.edu.ps", "high"),
    ("work: manager@company.com.ps", "high"),
    ("راسلني: info@mybusiness.ps", "high"),
    ("بريد العمل: hr@organization.org", "high"),
    ("my email address is test@domain.net", "high"),
    ("للتواصل: hello@startup.tech", "high"),
    ("business email: ceo@enterprise.com", "high"),
    ("ارسل على: admin@server.local", "high"),
    # هواتف إضافية
    ("رقم الجوال: 0592345678", "high"),
    ("phone number: 0598765432", "high"),
    ("رقم واتساب: 00970592345678", "high"),
    ("mobile number +970592345678", "high"),
    ("تليفوني: 0591111111", "high"),
    ("اتصل على: 0599222222", "high"),
    ("هاتفي: 0592333333", "high"),
    ("my number is 0598444444", "high"),
    ("contact number: +970595555555", "high"),
    ("رقمي 0596666666", "high"),
    ("phone: 0597777777", "high"),
    ("جوالي: 0593888888", "high"),
    ("للتواصل اتصل: 0594999999", "high"),
    ("رقم الهاتف المحمول: 059-000-0000", "high"),
    ("my cell: +1-800-555-0123", "high"),
    # عناوين ومعلومات شخصية
    ("عنوان السكن: حي الزيتون، غزة", "high"),
    ("أسكن في: شارع صلاح الدين، القدس", "high"),
    ("my home is at: 789 Pine Street, Chicago", "high"),
    ("العنوان: مخيم جنين، جنين", "high"),
    ("أقيم في: حي النصر، رام الله", "high"),
    ("سكني: شارع الوحدة، نابلس", "high"),
    ("living at: 321 Elm Street, Toronto", "high"),
    ("address: Flat 5, Building 3, Ramallah", "high"),
    ("تاريخ الميلاد: 1998/05/20", "high"),
    ("DOB: 1995-12-15", "high"),
    ("مواليد: 1992/08/30", "high"),
    ("born: March 15, 1997", "high"),
    ("birthday: 10/25/1993", "high"),
    ("اسم الأب: محمد علي", "high"),
    ("father name: Abdullah Hassan", "high"),
    ("اسم العائلة: الصفدي", "high"),
    ("family name: Al-Masri", "high"),
    ("الاسم الكامل: محمد أحمد الخليل", "high"),
    ("full name: Sarah Elizabeth Johnson", "high"),
    ("الاسم الثلاثي: أحمد محمود سالم", "high"),
    # حسابات بنكية — ⬆️ رُفعت لـ HIGH
    ("حسابي البنكي رقم 123456789", "high"),
    ("bank account number: 9876543210", "high"),
    ("رقم الحساب البنكي: 1234567890", "high"),
    ("my bank account is 0987654321", "high"),
    ("account number: 5678901234", "high"),
    ("رقم حسابي في البنك: 2345678901", "high"),
    ("bank account: 3456789012", "high"),
    ("الحساب البنكي رقم 4567890123", "high"),
    ("transfer to account number 6789012345", "high"),
    ("رقم الحساب للتحويل: 7890123456", "high"),
    # IP داخلية إضافية
    ("ip السيرفر: 192.168.10.50", "high"),
    ("database server: 192.168.0.100", "high"),
    ("internal address: 10.0.0.100", "high"),
    ("شبكة داخلية: 172.16.10.5", "high"),
    ("private ip: 192.168.50.200", "high"),
    ("server address: 10.10.0.50", "high"),
    ("internal server ip: 172.20.0.1", "high"),
    ("شبكة المكتب: 192.168.1.254", "high"),
    ("ip printer: 192.168.1.200", "high"),
    ("network camera: 10.0.0.200", "high"),
    # ================================================================
    # 🟠 MEDIUM — إضافة 111 جملة
    # ================================================================
    # روابط قواعد بيانات إضافية
    ("cassandra://user:pass@localhost:9042/keyspace", "medium"),
    ("elasticsearch://admin:secret@localhost:9200", "medium"),
    ("couchdb://admin:password@127.0.0.1:5984/db", "medium"),
    ("neo4j://neo4j:password@localhost:7687", "medium"),
    ("influxdb://admin:pass@localhost:8086/mydb", "medium"),
    ("DB_HOST=db.internal.company.com", "medium"),
    ("DATABASE_HOST=postgres.internal:5432", "medium"),
    ("REDIS_URL=redis://localhost:6379/0", "medium"),
    ("CELERY_BROKER_URL=amqp://guest:guest@localhost", "medium"),
    ("RABBITMQ_URL=amqp://user:pass@rabbitmq:5672", "medium"),
    ("connection: host=localhost port=5432 dbname=mydb", "medium"),
    ("db connection: mongodb://127.0.0.1:27017/prod", "medium"),
    ("مسار قاعدة البيانات: /var/lib/mysql/data", "medium"),
    ("backup database at: db.backup.server.com:3306", "medium"),
    ("replica db: mysql://replica:pass@slave:3306/db", "medium"),
    # بورتات وسيرفرات
    ("nginx running on port 80 and 443", "medium"),
    ("apache server port: 8080", "medium"),
    ("السيرفر يعمل على: 0.0.0.0:3000", "medium"),
    ("tomcat port: 8443", "medium"),
    ("websocket server: ws://localhost:8765", "medium"),
    ("grpc server: localhost:50051", "medium"),
    ("السيرفر الداخلي: app.internal:4000", "medium"),
    ("staging server: staging.internal.com:8080", "medium"),
    ("dev server running at localhost:4200", "medium"),
    ("test environment: test.local:3001", "medium"),
    ("microservice port: 8081", "medium"),
    ("api gateway: localhost:8000", "medium"),
    ("السيرفر الاحتياطي: backup.server:9000", "medium"),
    ("load balancer: lb.internal:80", "medium"),
    ("proxy server: proxy.company.com:3128", "medium"),
    # معلومات مالية عامة
    ("راتبي السنوي 60000 دولار", "medium"),
    ("monthly income: 3500 JD", "medium"),
    ("دخلي الشهري 8000 شيكل", "medium"),
    ("annual salary: $95,000", "medium"),
    ("معاشي التقاعدي 2000 دينار", "medium"),
    ("رقم الحساب الجاري: 1234-5678-9012", "medium"),
    ("savings account: 9876543210", "medium"),
    ("paypal account: user@paypal.com", "medium"),
    ("investment account: INV-123456789", "medium"),
    ("رصيد المحفظة: 15000 شيكل", "medium"),
    ("balance: $5,432.10", "medium"),
    ("رقم الشيك: 001234", "medium"),
    ("check number: 00567890", "medium"),
    ("tax ID: 123-45-6789", "medium"),
    ("رقم ملف الضريبة: TAX-123456", "medium"),
    ("vat number: PS123456789", "medium"),
    ("رقم ضريبة القيمة المضافة: 1234567890", "medium"),
    ("credit score: 750", "medium"),
    ("رصيد حسابي 25000 شيكل", "medium"),
    ("invoice total: $2,500 USD", "medium"),
    # معلومات طبية
    ("وزني 85 كيلو وطولي 175 سم", "medium"),
    ("blood pressure: 120/80", "medium"),
    ("ضغط الدم: 130/85", "medium"),
    ("sugar level: 110 mg/dL", "medium"),
    ("مستوى السكر في الدم: 95", "medium"),
    ("cholesterol: 180 mg/dL", "medium"),
    ("allergy to: penicillin", "medium"),
    ("عندي حساسية من: الغلوتين", "medium"),
    ("my BMI is 24.5", "medium"),
    ("heart rate: 72 bpm", "medium"),
    ("نبضات القلب: 80 في الدقيقة", "medium"),
    ("prescription: amoxicillin 500mg twice daily", "medium"),
    ("الدواء الموصوف: ميتفورمين 850 ملغ", "medium"),
    ("vaccination record: COVID-19 Pfizer 2 doses", "medium"),
    ("تاريخ التطعيم: لقاح كورونا فايزر", "medium"),
    ("الحالة الصحية: ضغط دم مرتفع", "medium"),
    ("medical condition: type 1 diabetes", "medium"),
    ("hospital visit: 15/03/2024", "medium"),
    ("آخر زيارة للطبيب: 2024/01/20", "medium"),
    ("lab results: hemoglobin 13.5", "medium"),
    # معلومات تقنية حساسة
    ("server version: Apache/2.4.51", "medium"),
    ("php version: 7.4.3", "medium"),
    ("إصدار قاعدة البيانات: MySQL 8.0.28", "medium"),
    ("OS version: Ubuntu 20.04.3 LTS", "medium"),
    ("kernel: Linux 5.4.0-74-generic", "medium"),
    ("framework version: Laravel 9.0", "medium"),
    ("node version: v16.14.0", "medium"),
    ("python version: 3.9.7", "medium"),
    ("SSL certificate expires: 2024-12-31", "medium"),
    ("hosting account: cpanel user mysite123", "medium"),
    ("ftp server: ftp.mysite.com port 21", "medium"),
    ("smtp server: mail.company.com:587", "medium"),
    ("imap server: imap.gmail.com:993", "medium"),
    ("admin panel: /wp-admin login page", "medium"),
    ("phpmyadmin at: localhost/phpmyadmin", "medium"),
    ("jenkins running at: ci.internal.com:8080", "medium"),
    ("grafana dashboard: monitoring.internal:3000", "medium"),
    ("kibana: logs.internal.company.com:5601", "medium"),
    ("prometheus: metrics.internal:9090", "medium"),
    ("sonarqube: code.internal:9000", "medium"),
    ("gitlab instance: git.company.internal", "medium"),
    ("docker registry: registry.internal:5000", "medium"),
    ("kubernetes api: k8s.internal:6443", "medium"),
    ("vault address: vault.internal:8200", "medium"),
    ("consul: service.mesh.internal:8500", "medium"),
    ("nexus repository: repo.internal:8081", "medium"),
    # ================================================================
    # 🟢 SAFE — إضافة 90 جملة
    # ================================================================
    # برمجة إضافية
    ...
    ("اشرح لي مفهوم الـ virtual DOM", "safe"),
    ("what is webpack and how does it work?", "safe"),
    ("كيف أعمل responsive design؟", "safe"),
    ("explain SOLID principles in OOP", "safe"),
    ("ما هو الـ design pattern المناسب هنا؟", "safe"),
    ("how to implement a linked list in Python?", "safe"),
    ("اشرح خوارزمية الـ bubble sort", "safe"),
    ("what is the difference between stack and heap?", "safe"),
    ("كيف تعمل الـ cookies والـ sessions؟", "safe"),
    ("explain how CORS works", "safe"),
    ("ما هو الفرق بين REST و GraphQL؟", "safe"),
    ("how to write clean code?", "safe"),
    ("اشرح لي مفهوم الـ dependency injection", "safe"),
    ("what is test driven development?", "safe"),
    ("كيف أعمل code review صحيح؟", "safe"),
    ("explain agile methodology", "safe"),
    ("ما هو الفرق بين Scrum و Kanban؟", "safe"),
    ("how to use TypeScript with React?", "safe"),
    ("اشرح لي مفهوم الـ serverless", "safe"),
    ("what is edge computing?", "safe"),
    ("كيف تعمل الـ blockchain؟", "safe"),
    # أسئلة AI
    ("what is the difference between GPT-3 and GPT-4?", "safe"),
    ("كيف يعمل نموذج الـ diffusion للصور؟", "safe"),
    ...
    ("اقترح لي كتب للقراءة", "safe"),
    ("how to learn a new language quickly?", "safe"),
    ("ما هي أفضل طريقة للادخار؟", "safe"),
    ("help me write a birthday message", "safe"),
    ("كيف أحضّر لمقابلة عمل؟", "safe"),
    ("what questions should I ask in a job interview?", "safe"),
    ("اكتب لي CV احترافي", "safe"),
    ("how to start a small business?", "safe"),
    ("ما هي أفضل منصات التعلم الإلكتروني؟", "safe"),
    ("اشرح لي كيف أستثمر أموالي", "safe"),
    ("what is the best diet for weight loss?", "safe"),
    ("كيف أبدأ رياضة المشي؟", "safe"),
    ("help me plan a healthy meal", "safe"),
    ("ما هي فوائد النوم الكافي؟", "safe"),
    ("how to reduce stress and anxiety?", "safe"),
    ("اشرح لي فوائد التأمل", "safe"),
    ("what are the best travel destinations?", "safe"),
    ("كيف أتعامل مع ضغط العمل؟", "safe"),
    # محادثات عادية وآمنة
    ("أنا أدرس هندسة البرمجيات", "safe"),
    ("I am a software engineering student", "safe"),
    ("أعمل مطور ويب منذ 3 سنوات", "safe"),
    ...
    ("أنا في سنتي الثالثة بالجامعة", "safe"),
    ("I am studying computer science", "safe"),
    ("أريد أن أتعلم تطوير تطبيقات الموبايل", "safe"),
    ("I want to become a full stack developer", "safe"),
    ("كيف حالك؟ أحتاج مساعدة في مشروع", "safe"),
    ("can you help me with my homework?", "safe"),
    ("أحتاج مساعدة في فهم الخوارزميات", "safe"),
    ("I need help understanding recursion", "safe"),
    ("ما رأيك في لغة البرمجة Rust؟", "safe"),
    ("which is better Python or JavaScript?", "safe"),
    ("اشرح لي الفرق بين iOS و Android development", "safe"),
    ...
    ("اشرح لي مفهوم الـ open source", "safe"),
    ("what is the difference between Linux and Windows?", "safe"),
    ("كيف أختار بين Mac وWindows للبرمجة؟", "safe"),
    ("what IDE should I use for Python?", "safe"),
    ("اشرح لي كيف أستخدم VS Code", "safe"),
    ("how to set up a development environment?", "safe"),
]
